...
const ADMIN_ONLY_REASON = "Group editing is only available to administrators.";

export default function GroupsPage() {
    const {token, isAdmin} = useAuth();
    const {data, loading, saving, save, reload, currentDocText, version} =
        useConfigResource<GroupsConfig>("groups", {groups: []});
    ...
    function openNew() {
        const d: GroupT = {name: "", description: "", conditions: []};
    ...
    function openEdit(i: number) {
        ...
        // conditions is optional in the YAML; the builder needs a real array.
        const d: GroupT = {...g, conditions: g.conditions ?? []};
        baseline.current = JSON.stringify(d);
        setDraft(d);
        setEditIndex(i);
        setNameError(null);
    ...
    const dirty = modalOpen && JSON.stringify(draft) !== baseline.current;
    useBeforeUnload(dirty);
    const closeModal = () =>
        confirmDiscard({dirty, what: "this group", onConfirm: () => setModalOpen(false)});
    ...
    return (
        <Stack gap="lg">
            ...
            {loading ? (
                ...
                <Stack gap="sm">
                    {groups.map((g, i) => (
                        <GlassCard key={g.name} withBorder padding="md">
                            <Group justify="space-between" align="flex-start" wrap="nowrap">
                                <Stack gap={6} style={{flex: 1, minWidth: 0}}>
                                    ...
                                    <Group gap={6} wrap="wrap">
                                        {(g.conditions?.length ?? 0) === 0 && !g.include_devices?.length ? (
            ...
            >
                <Stack gap="md">
                    ...
                    <Box>
                        <UnstyledButton onClick={() => setCherryOpen((o) => !o)}>
                            <Group gap={4}>
                                <IconChevronRight
                                    size={14}
                                    style={{
                                        transform: cherryOpen ? "rotate(90deg)" : "none",
                        ...
                        <Collapse expanded={cherryOpen}>
                            <Group grow align="flex-start" gap="md" mt="xs">
                                <Box>
                                    ...
                                    <DevicePicker
                                        ...
                                        onChange={(v) => setDraft({
                                            ...draft,
                                            include_devices: v.length ? v : undefined
                                        })}
                                        color="teal"
                                    />
                                </Box>
                                <Box>
                                    ...
                                    <DevicePicker
                                        ...
                                        onChange={(v) => setDraft({
                                            ...draft,
                                            exclude_devices: v.length ? v : undefined
                                        })}
                                        color="red"
                                    />
                                </Box>
                            ...
                            <Text fz="xs" c="dimmed" mt={4}>
                                Excluded devices stay out of this group even when they match the conditions.
                            </Text>
                            <Text fz="xs" c="dimmed">
                                Included devices stay in it even when they do not.
                            </Text>
                        </Collapse>
                    </Box>
                    ...
                    {(draft.conditions?.length ?? 0) === 0 && !draft.include_devices?.length && (
                        <Group gap={6}>
                    ...
                    <Stack gap="xs">
                        <Text fz="xs" c="dimmed">
                            ...
                            groups, the first group from the top that defines a template wins.
                        </Text>
                        <NameTemplateInput
                            label="Name template"
                            placeholder="e.g. CART-A-{serial}"
                        ...
                        {selfReferential && (
                            <Group gap={6} wrap="nowrap" align="flex-start">
                                ...
                                />
                                <Text fz="xs" c="orange.7">
                                    {"{hostname}"} is the device&apos;s own name, so this template renames the
                                    device after itself, over and over. Use a different variable like{" "}
                                    {"{serial}"}.
                                </Text>
                            </Group>
                        )}
                        ...
                        <Switch
                            ...
                            description="Names matching devices as they enroll, and only if they have no name yet. With this off, the template is a suggestion in the rename box and nothing applies it for you."
                        />
                    </Stack>
                        ...
                        <Tooltip label={ADMIN_ONLY_REASON} withArrow disabled={isAdmin}>
                            <Box>
                                <Button
                                    onClick={handleSave}
                                    loading={saving}
                                    disabled={saving || selfReferential || !isAdmin}
                                >
