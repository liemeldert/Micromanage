...
/**
 * Multi-select list of devices by serial, for cherry-picking include/exclude
 * cohorts. Searchable by serial / hostname / model; shows the managed name so
 * a human can recognise the device. `color` tints the selection (teal for
...
export function DevicePicker({
  ...
  // Serials that are selected but not among the loaded devices (e.g. a
  // pre-provisioned/offline device) -- keep them visible so they aren't dropped.
              ...
              {filtered.map((d) => {
                ...
                return (
                  <Group
                    ...
                    style={{ cursor: "pointer", background: on ? `var(--mantine-color-${color}-light)` : undefined }}
