...
import { rolloutCoverage, type Rollout } from "../../../lib/config";

/**
 * Editor for a gradual (wave-based) rollout: each interval another `percent`
 * of scoped devices become eligible; weekends can be skipped. Shows the wave
 * schedule so the operator sees exactly when each cohort goes out.
 */
export function RolloutEditor({
  ...
  const intervalPreset =
    r.interval_hours === 24 ? "day" : r.interval_hours === 168 ? "week" : "custom";

  // Project the wave schedule from a fresh start (server stamps the real start
  // on save). Cap at the first 100%-covering wave.
            ...
            <Stack gap={2}>
              ...
              <SegmentedControl
                ...
                onChange={(v) =>
                  onChange({
                    ...r,
                    interval_hours: v === "day" ? 24 : v === "week" ? 168 : r.interval_hours,
                  })
                }
                data={[
                  { label: "Day", value: "day" },
...
// Rough human duration; when skipping weekends, day-based intervals are
// labelled in business days for clarity.
...
// Re-export so integrators can show current coverage if they have a real start.
export { rolloutCoverage };
