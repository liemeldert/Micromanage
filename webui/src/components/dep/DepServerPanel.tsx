...
export function DepServerPanel({
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({
        color: "red",
        title: "Could not load DEP server",
  ...
  async function assignSelected() {
    ...
    setBusy(true);
    try {
      const { results } = await api.assignDepProfile(token, server.id, assignProfileId, serials);
      const ok = Object.values(results).filter((r) => r === "SUCCESS").length;
      notifications.show({
        color: ok === serials.length ? "green" : "orange",
        title: "Profile assigned",
        message: `${ok}/${serials.length} succeeded.`,
      });
      setSelected(new Set());
      await reload();
    } catch (e) {
      ...
      children: (
        <Stack gap="xs">
          <Text size="sm">
            ...
            organization&apos;s Automated Enrollment in ABM/ASM. This cannot be undone here —
            the device would have to be re-added to ABM by Apple/your reseller.
          </Text>
            ...
            <Menu position="bottom-end" withinPortal>
              ...
              <Menu.Dropdown>
                ...
                <FileButton onChange={uploadTokenRenewal} accept=".p7m,application/pkcs7-mime">
                  {(props) => (
                    <Menu.Item {...props} leftSection={<IconUpload size={14} />} closeMenuOnClick={false}>
                      Renew server token…
                    </Menu.Item>
                  )}
                </FileButton>
                ...
                <Table.Tr>
                  <Table.Th w={36}>
                    <Checkbox
                      ...
                      onChange={(e) =>
                        setSelected(
                          e.currentTarget.checked ? new Set(devices.map((d) => d.id)) : new Set(),
                        )
                      }
